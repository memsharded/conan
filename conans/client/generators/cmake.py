...
class CMakeGenerator(Generator):
    ...
    if(RUNTIME_DEPRECATION_WARNING)
        message(WARNING "The 'compiler.runtime' setting has been deprecated and will be removed. Please use 'msvcrt', instead.")
    endif()
...
macro(conan_flags_setup)
    ...
    if(DEFINED CONAN_STDLIB)
        message(STATUS "Conan C++ stdlib: ${CONAN_STDLIB}")
        if(CONAN_STDLIB STREQUAL "libstdc++" OR CONAN_STDLIB STREQUAL "libstdc++11" )
            set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -stdlib=libstdc++")
        elseif(CONAN_STDLIB STREQUAL "libc++")
            set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -stdlib=libc++")
        endif()
        if(CONAN_STDLIB STREQUAL "libstdc++11")
            add_definitions(-D_GLIBCXX_USE_CXX11_ABI=1)
        elseif(CONAN_STDLIB STREQUAL "libstdc++")

...
from conan.internal.util.files import save, load, is_dirty, set_dirty
...
def test_cache_restore_overwrites_existing_contents():
    """ restore is not incremental: any manual change to an already restored revision is
    discarded and replaced again by the contents of the archive """
    _, cache_path = _save_built_package()
...
def test_cache_restore_read_only_files_in_place():
    ...
    # The read-only contents are removed and extracted again without any permission error
...
def test_cache_restore_metadata_overwrites_existing_contents():
    """ metadata restore is not incremental either: it replaces the previous metadata contents,
    same as recipes and packages """
    c = TestClient()
    ...
    assert not os.path.exists(os.path.join(metadata, "logs", "mine.txt"))
